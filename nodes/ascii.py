...
class ASCII:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "font_color": ("STRING", {"default": "#c7205b"}),
                "font_color2": ("STRING", {"default": "#00ff61"}),
                "resolution": ("INT", {"default": 137, "min": 1}),
                "threshold": ("INT", {"default": 0, "min": 0, "max": 255}),
                "invert": ("BOOLEAN", {"default": True}),
                "randomness": ("INT", {"default": 15, "min": 0, "max": 100}),
                "text_type": (["Random Text", "Input Text"], {}),
                "text_input": ("STRING", {"default": "pxlpshr", "multiline": False}),
            },
        }
    ...
    def _tensor_to_pil(self, image):
        ...
        out = []
        if arr.ndim == 4:
            out.extend([self._array_to_pil(t) for t in arr])
        else:
    ...
    def _make_ascii(self, pil_img, background, fc1, fc2, res, thr, inv, rnd, ttype, tinput):
        w, h = pil_img.size
        ...
        chars = list(tinput) if ttype == "Input Text" and tinput else list(" .:-=+*#%@")
